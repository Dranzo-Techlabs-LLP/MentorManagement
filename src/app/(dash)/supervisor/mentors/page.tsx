...
/** Mentors that report to the signed-in supervisor. */
export default async function SupervisorMentorsPage({
  searchParams,
}: {
  ...
  const [mentors, institutions, self] = await Promise.all([
    prisma.user.findMany({
      where,
      orderBy: { name: "asc" },
      include: { _count: { select: { studentsAsMentor: true, mentoredSessions: true } } },
    }),
  ...
  // Scoped to this supervisor only — new/edited mentors can only report to themselves,
  // so a supervisor can't reassign a mentor onto someone else's team.
  const supervisors = self ? [self] : [];
  ...
  return (
    <>
      <PageHeader
        title="My Mentors"
        subtitle="Mentors under your supervision"
        action={
      ...
      <Panel>
        <DataTable
          rows={mentors}
          getKey={(m) => m.id}
          empty="No mentors assigned to you."
          columns={[
            {
              header: "Mentor",
              cell: (m) => (
