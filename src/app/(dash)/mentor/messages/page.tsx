...
export default async function MentorMessagesPage() {
  ...
  const [inbox, sent, mentees, me] = await Promise.all([
    ...
    prisma.student.findMany({
      where: { mentorId, parentId: { not: null } },
      select: { parent: { select: { id: true, name: true, role: true } } },
    }),
  ...
  // recipients: parents of my mentees + my manager (supervisor / chief), de-duplicated
  const recipientMap = new Map<string, { id: string; name: string; role: string }>();
  for (const m of mentees) {
    if (m.parent) recipientMap.set(m.parent.id, m.parent);
  }
  if (me?.manager) recipientMap.set(me.manager.id, me.manager);
