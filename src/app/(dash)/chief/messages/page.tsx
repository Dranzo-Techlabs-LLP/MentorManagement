...
/** Chief mentor inbox + sent. Compose targets all supervisors and super admins. */
export default async function ChiefMessagesPage() {
  ...
  const [inbox, sent, recipients] = await Promise.all([
    ...
    prisma.user.findMany({
      where: { id: { not: session.userId }, role: { in: ["SUPERVISOR", "SUPER_ADMIN"] } },
      orderBy: { name: "asc" },
  ...
  return (
    <>
      <PageHeader
        title="Messages"
        subtitle={`Chief mentor inbox · ${unread} unread`}
