...
export default async function ParentMessagesPage() {
  ...
  // Recipients = unique mentors of this parent's children.
  const mentors = Array.from(
    new Map(
      children.filter((c) => c.mentor).map((c) => [c.mentor!.id, c.mentor!]),
    ).values(),
  );
  ...
  return (
    <>
      <PageHeader
        ...
        action={<ComposeModal mentors={mentors.map((m) => ({ id: m.id, name: m.name, role: m.role }))} />}
      />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        >
          {inbox.length === 0 ? (
            <EmptyState title="No messages" hint="Messages from mentors appear here." />
          ) : (
...
function ComposeModal({ mentors }: { mentors: { id: string; name: string; role: string }[] }) {
  return (
    <Modal
    ...
    >
      <ActionForm action={sendMessage} className="space-y-4">
          <Field label="To (mentor)">
            <select name="recipientId" className="input" required defaultValue="">
              <option value="" disabled>
                {mentors.length ? "Select a mentor…" : "No mentors available"}
              </option>
              {mentors.map((m) => (
