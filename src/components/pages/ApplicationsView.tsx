...
const TAB_STATUS: Record<string, ApplicationStatus | undefined> = {
  pending: "PENDING", approved: "APPROVED", rejected: "REJECTED", all: undefined,
};

const PAGE_SIZE = 10;

/**
 * Parent (enrollment) applications — shared by every workspace that can be
 * granted the `applications` resource. These are program-wide intake records
 * with no per-user scoping, so the only gate is the permission matrix.
 */
...
function ApproveModal({
  id, studentName, mentors, institutions,
}: {
  ...
  return (
    ...
    >
      <ActionForm action={approveApplication} className="space-y-4" successMessage="Application approved.">
...
function RejectModal({ id, studentName }: { id: string; studentName: string }) {
  return (
    ...
    >
      <ActionForm action={rejectApplication} className="space-y-4" successMessage="Application rejected.">
        <input type="hidden" name="id" value={id} />
        <p className="text-sm text-slate-600">
